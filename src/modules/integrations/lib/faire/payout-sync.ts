...
async function processFairePayout(opts: {
  ...
  // ValidationException on 56/224 payouts ("debits must equal
  // credits"). Empirical analysis of Faire's payout_costs shows
  // totalPayout = netOrderTotal - paymentFee (commission for Direct
  // is 0), so the journal balances on those three lines alone:
  //
  //   debit  receivables_holding  = totalPayout      = netOrderTotal - paymentFee
  //   debit  payment_fee_expense  = paymentFee
  //   credit deferred_revenue     = -netOrderTotal
  //   sum: (netOrderTotal - paymentFee) + paymentFee + (-netOrderTotal) = 0 ✓
  //
  // The shippingReimbursement field (maker_cost on the shipment) is
  // NOT in Faire's totalPayout — they don't pay it back through this
  // payout flow. The shipping expense already hit the books when the
  // maker paid for the label; if Faire ever reimburses separately,
  // that needs its own journal (debit bank, credit shipping expense).
  // Including it here as an unmatched credit broke 56 journals.
