...
export function getXeroAuthUrl(): string | null {
  ...
  //   accounting.reports.banksummary.read - bank reports (BankStatement /
  //                                 BankSummary) for the daily Faire rec digest.
  //                                 NOTE: this granular system names report
  //                                 scopes PER REPORT (accounting.reports.
  //                                 <report>.read; probed 2026-08-05 — legacy
  //                                 accounting.reports.read returns
  //                                 invalid_scope, as do all *bankstatement*
  //                                 spellings; banksummary is the valid bank
  //                                 one and covers the statement report).
  ...
  const scopes = process.env.XERO_SCOPES ||
    "openid profile email offline_access accounting.manualjournals accounting.banktransactions accounting.contacts accounting.invoices accounting.settings.read accounting.reports.banksummary.read files";
