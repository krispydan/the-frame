...
// Coarse status → dot color (greens = further along).
const STATUS_DOT: Record<string, string> = {
  idea: "bg-muted-foreground/40",
  themed: "bg-muted-foreground/60",
  copy_pending: "bg-amber-400",
  copy_review: "bg-amber-500",
  image_pending: "bg-blue-400",
  image_review: "bg-blue-500",
  preview_ready: "bg-violet-500",
                    ...
                    {items.map((c) => (
                      ...
                      >
                        <div className="flex items-center gap-1">
                          ...
                          <span className="truncate">{c.subject ?? c.heroHeadline ?? "(untitled)"}</span>
                        </div>
                      </Link>
                    ))}
                  </div>
