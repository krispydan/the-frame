 ...
 * POST ?confirm=1 {action:'verify', limit, minScore}
 *                                        → NeverBounce the top N (spends credits)
 *
...
export async function POST(req: NextRequest) {
  ...
  const stats = await verifyProspectEmails({
    companyIds: needing.map((s) => s.companyId),
  ...
  return NextResponse.json({
    ...
    submittedForVerification: needing.length,
    stats,
  });
}
