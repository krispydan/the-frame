...
/**
 * GET /api/v1/orders/lookup?number=RDVK4VZ3NX  (also accepts ?accountId=…)
 *
...
export async function GET(req: NextRequest) {
  ...
  // For comparison: which company does the customer account point at?
  ...
  if (!number) {
    return NextResponse.json({ account, error: "pass ?number=<order number>" });
  }

  // Match the order number generously: exact, with a leading '#', a LIKE, or
  // by Shopify/Faire external id.
  const like = `%${number}%`;
  ...
  // If the order lands on a different company than the account, surface any
  // sibling companies that look like the SAME retailer (duplicate records),
  // to make merging obvious.
