...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (platform) conditions.push(eq(adCampaigns.platform, platform as (typeof adCampaigns.platform.enumValues)[number]));
    if (status) conditions.push(eq(adCampaigns.status, status as (typeof adCampaigns.status.enumValues)[number]));
