...
**Test method note:** the audit ran from a throwaway scratch script. It has since been made **permanent** as `src/__tests__/marketing/render-email.test.ts` (17 tests, run by `npm test`): it parses the rendered HTML with jsdom and asserts no style truncation across every variant combination, dead-CTA omission, the pullquote fallback, disabled-section omission, escaping, hero legibility, and structural integrity. Verified to fail when the critical bug is reintroduced — so the regression that three eyeball passes missed can never silently return.
